...
in_dir=wav_vcc20_24kHz_dv
out_dir=wav_cv_point_dv

mkdir -p $out_dir

for spk_src in ${spks_src[@]};do
    find ${in_dir} -name "*.wav" | grep "\/${spk_src}"  |  sort > tmp_${spk_src}_dv.list
    spk_idx=1
    for spk in ${spks[@]};do
        if [[ "$spk" =~ .*"TE".* ]] || \
                [[ "$spk" =~ .*"TF".* ]] || \
            ...
            done < tmp_${spk_src}_dv.list
        fi
        spk_idx=$(( ${spk_idx}+1  ))
    done
    rm -f tmp_${spk_src}_dv.list
done
